...
export function useProductSpreadsheet(options: UseProductSpreadsheetOptions = {}) {
  ...
  const persistDraft = useCallback(
    async (row: SpreadsheetRow) => {
      ...
      const attemptCreate = async (draft: SpreadsheetRow) => {
        const { data: created } = await create.mutateAsync(buildCreatePayload(draft));
        clientRowIdRef.current.set(created.id, draft._clientRowId);
        setDraftRows((prev) => prev.filter((d) => d._clientRowId !== draft._clientRowId));
        upsertProductInAllListCaches(qc, created);
        invalidateProductStats(qc);

        const followUp = buildFollowUpUpdate(draft, created);
        if (followUp) {
      ...
      try {
        ...
        if (isSkuConflict) {
          const retried: SpreadsheetRow = {
            ...row,
            sku: generateSkuCandidate(),
          };
          setDraftRows((prev) =>
            prev.map((d) =>
              d._clientRowId === row._clientRowId ? retried : d,
            ),
          );
          try {
            await attemptCreate(retried);
            return;
          } catch {
